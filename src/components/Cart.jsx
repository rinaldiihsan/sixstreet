      ...
      >
        <div className="flex items-center">
          ...
          <div className="flex flex-col">
            <h4 className="md:text-lg font-overpass mb-2 font-medium">
              {product.product_name}
            </h4>
            <div className="flex gap-x-5 mb-2">
              {product.size && (
                <h4 className="md:text-lg font-overpass">
                  Size: {product.product_size}
                </h4>
              )}
              <p className="md:text-lg text-[#333333] font-overpass">
                Quantity: {product.quantity}
              </p>
            </div>
            <div className="flex gap-x-2 mb-2">
              <button className="quantity-btn border border-[#333333] px-3 py-1 font-overpass">
                Edit Items
              </button>
              <button
                className="quantity-btn border border-[#333333] px-3 py-1 font-overpass"
                onClick={() => handleRemoveItem(product)}
              >
                Remove
              </button>
            </div>
            <p className="block md:hidden font-overpass">
              {formatPrice(product.product_price * product.quantity)}
            </p>
          </div>
        </div>
        <p className="hidden md:block text-lg font-overpass">
          {formatPrice(product.product_price * product.quantity)}
        </p>
      </div>
    ));
    ...
    <>
      {cartOpen && (
        <div className={`cart ${cartOpen ? "cart-open" : ""}`}>
          <div
            className={`overlay-cart ${cartOpen ? "overlay-open" : ""}`}
            onClick={() => showCart(false)}
          >
            <div className="absolute inset-0 bg-[#333333] opacity-30"></div>
          </div>
          <motion.div
            className={`cart-content ${cartOpen ? "cart-content-open" : ""}`}
            initial="closed"
          ...
          >
            <div className="flex-1 flex flex-col py-10 overflow-y-auto">
              <div className="flex items-center justify-between w-full px-10">
                <h2 className="font-garamond text-2xl font-semibold">
                  Shopping Cart
                </h2>
              ...
              <div className="mt-5 px-8 space-y-2">
                {cartItems.length > 0 ? (
                  ...
                  <p className="text-lg text-gray-600 font-overpass">
              ...
              {cartItems.length > 0 && (
                <div className="mt-5 px-8">
                  <button
                    ...
                    className="block w-full bg-[#333333] text-white py-3 text-center font-semibold rounded font-overpass"
                    type="button"
                  >
                    Checkout
                  </button>
      ...
      {showLoginPopup && (
        <div className="fixed inset-0 z-[99991] flex items-center justify-center bg-black bg-opacity-50 px-5">
          <div className="bg-white p-6">
            <h3 className="text-xl mb-4 font-garamond">
              You need to be logged in to proceed to checkout.
            </h3>
            <div className="flex justify-end">
              <button
                onClick={handleLoginClick}
                className="bg-[#333333] text-white py-2 px-6 font-garamond mr-2"
              >
                Login
              </button>
              <button
                onClick={() => setShowLoginPopup(false)}
                className="bg-gray-200 py-2 px-4 font-garamond"
              >
                Cancel
              </button>
            </div>
