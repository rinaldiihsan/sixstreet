    ...
    <>
      <div className="relative">
        ...
        <div className="absolute bottom-[5%] md:bottom-[10%] left-0 text-white px-5 md:px-10 flex flex-col items-center md:items-start">
          <div className="md:max-w-[40rem] mb-2 text-center md:text-left">
            ...
            <p className="capitalize font-garamond font-medium text-lg md:text-2xl">
              Lorem ipsum dolor sit amet consectetur. Vehicula ac bibendum elit
              varius auctor posuere.
            </p>
          </div>
          <Link
      ...
      <div className="max-w-[115rem] mx-5 md:mx-auto flex gap-y-8 flex-row flex-wrap justify-between mb-10">
        {isLoading ? (
          ...
          products.some((item) => item.item_category_id === 18200) ? (
          Object.values(
            products
      ...
      <div className="max-w-[115rem] mx-5 md:mx-auto flex gap-y-8 flex-row flex-wrap justify-between mb-10">
        {isLoading ? (
          ...
          products.some((item) =>
            [5472, 999, 1013, 12780, 12803].includes(item.item_category_id)
          ) ? (
          Object.values(
            products
              .filter((item) =>
      ...
      <div className="max-w-[115rem] mx-5 md:mx-auto flex gap-y-8 flex-row flex-wrap justify-between mb-10">
        {isLoading ? (
          ...
          products.some(
            (item) =>
              item.item_category_id === 17866 || item.item_category_id === 7332
          ) ? (
          Object.values(
            products
              .filter(
                (item) =>
                  item.item_category_id === 17866 ||
                  item.item_category_id === 7332
              )
              .flatMap((item) => ({
                ...item.variants[0],
                item_group_id: item.item_group_id,
      ...
      <div className="max-w-[115rem] items-center justify-center mx-5 md:mx-auto flex flex-col gap-y-8 md:flex-row md:flex-wrap md:justify-between mb-10">
        {news.map((item) => (
          <div key={item.id} className="flex flex-col gap-y-3 ">
            ...
            <Link
              to={`/news/${encodeURIComponent(item.judulberita)}`}
              className="mb-5"
            >
              <img
                src={getImageUrl(item)}
                alt={item.judulberita}
                className="w-[38rem] object-cover"
              />
            </Link>
            <div className="flex flex-col gap-y-2 items-center">
              {/* Tambahkan items-center di sini */}
              <h2 className="capitalize font-garamond font-extrabold text-xl max-w-[37rem] text-center md:text-left md:self-start">
                {item.judulberita}
              </h2>
              <Link
                to={`/news/${encodeURIComponent(item.judulberita)}`}
                className="w-full md:w-auto text-center md:text-left md:self-start font-garamond py-1 md:py-2 px-11 md:px-10 border border-[#333333] text-xl"
              >
                View Blog
              </Link>
            </div>
