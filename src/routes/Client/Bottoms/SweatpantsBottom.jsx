...
import assetBannerSweatpants from '../../../assets/banner/sweatpants.webp';

const SweatpantsBottom = () => {
  ...
  const sweetpantsCategoryIds = [18205];
  ...
  const getSweatpantsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with t-shirts category ID
      if (!sweetpantsCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sweetpantsProducts = getSweatpantsProducts(response.data.data);
        setProducts(sweetpantsProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerSweatpants} alt="Hero T-Shirt" className="w-full h-full md:h-auto mb-6" />
...
export default SweatpantsBottom;
