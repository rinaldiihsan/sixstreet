...
const OrderHistory = () => {
  ...
  // Function untuk mendapatkan style status yang lebih lengkap
  ...
  // Function untuk mendapatkan display text status
                ...
                groupedArray.map((transaction) => (
                  <tr key={transaction.transaction_uuid} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusStyle(transaction.status)}`}>{getStatusDisplayText(transaction.status)}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {transaction.status.toLowerCase() === 'pending' ? (
                        <div className="flex space-x-2">
                          <button onClick={() => handlePayment(transaction)} className="text-white bg-[#333333] hover:bg-[#444444] px-3 py-1.5 rounded text-sm font-medium transition-colors duration-200">
                            Pay
                          </button>
                          <button onClick={() => handleCancel(transaction)} className="text-white bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded text-sm font-medium transition-colors duration-200">
                            Cancel
                          </button>
                        </div>
                      ) : (
                        <button onClick={() => handleDetail(transaction)} className="text-white bg-blue-500 hover:bg-blue-600 px-3 py-1.5 rounded text-sm font-medium transition-colors duration-200">
                          Detail
                        </button>
                      )}
                    </td>
