...
const Yeezy = () => {
  ...
  const getYeezyProducts = (products) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const yeezyProducts = getYeezyProducts(response.data.data);
        setProducts(yeezyProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={heroYeezy} alt="Hero Adidas" className="w-full h-full md:h-auto mb-6" />
