...
const ProductDetail = ({ userId, isLoggedIn }) => {
  ...
  const fetchProductGroup = async (token) => {
    try {
      ...
      // 1. Fetch catalog data untuk gambar terbaru
      ...
      // 2. Ambil item_id dari variant pertama untuk fetch detail
      let firstItemId;
      if (groupResponse.status === 200 && groupResponse.data.length > 0) {
        const catalogData = groupResponse.data[0];

        // Set images dari catalog
        if (catalogData.images && catalogData.images.length > 0) {
          setProductImages(catalogData.images);
          setProductImage(catalogData.images[0].thumbnail);
        }
      ...
      // 3. Fetch detail item untuk description dan SKU terbaru
      if (firstItemId) {
        const itemResponse = await axios.get(
          `${apiUrl}/inventory/items/${firstItemId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (itemResponse.status === 200) {
          ...
          // Set product info dari item detail
          setProduct({
          ...
          if (itemData.product_skus && itemData.product_skus.length > 0) {
            setProductSkus(itemData.product_skus);
            ...
            const availableSku = itemData.product_skus.find(
              (sku) => sku.end_qty > 0
            );
  ...
  useEffect(() => {
    if (productSkus.length > 0) {
      ...
      if (availableSkus.length > 0) {
        ...
        if (
          firstAvailableSku.variation_values &&
          firstAvailableSku.variation_values[0]
        ) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const existingItem = cartItems.find(
        (item) =>
          item.product_id === productToCart.product_id &&
      ...
      <div className="w-full flex flex-col lg:flex-row mb-6 gap-x-11 justify-center space-y-5 lg:space-y-0">
        ...
        <div className="lg:w-[40rem] flex flex-col gap-y-3">
          ...
          {getAvailableSKUs().length > 0 ? (
            ...
            <p className="text-lg font-semibold font-overpass text-[#333333] text-center">
              Out of Stock
            </p>
          )}
          ...
          <button
            ...
            className={`${
              getAvailableSKUs().length === 0
                ...
                : "bg-[#333333] hover:bg-[#fff] hover:text-[#333] hover:border hover:border-[#333]"
