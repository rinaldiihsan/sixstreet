...
const ShirtsTops = () => {
  ...
  // Category IDs for shirts products
  const shirtsCategoryIds = [18198, 18209];
  ...
  // Helper function to get Shirts products by category ID
  const getShirtsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with shirts category IDs
      if (!shirtsCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const shirtsProducts = getShirtsProducts(response.data.data);
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Shirts products found</p>}
              </>
            )}
          </div>
        </div>
