...
const JacketsTops = () => {
  ...
  // Category IDs for jackets products
  const jacketsCategoryIds = [18218, 18210, 18216];
  ...
  // Helper function to get Jackets products by category ID
  const getJacketsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with jackets category IDs
      if (!jacketsCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const jacketsProducts = getJacketsProducts(response.data.data);
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Jackets products found</p>}
              </>
            )}
          </div>
        </div>
