...
const HoodiesTops = () => {
  ...
  // Category ID for hoodies products
  const hoodiesCategoryId = [18215];
  ...
  // Helper function to get Hoodies products by category ID
  const getHoodiesProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with hoodies category ID
      if (!hoodiesCategoryId.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const hoodiesProducts = getHoodiesProducts(response.data.data);
        setProducts(hoodiesProducts);
      } else {
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Hoodies products found</p>}
              </>
            )}
          </div>
        </div>
