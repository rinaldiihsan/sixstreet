...
import assetBannerPoloShirts from '../../../assets/banner/polo shirts.webp';

const PoloShirtsTops = () => {
  ...
  // Category ID for polo shirts products
  const poloShirtsCategoryId = [18199];
  ...
  // Helper function to get Polo Shirts products by category ID
  const getPoloShirtsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with polo shirts category ID
      if (!poloShirtsCategoryId.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const poloShirtsProducts = getPoloShirtsProducts(response.data.data);
        setProducts(poloShirtsProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerPoloShirts} alt="Hero Polo Shirts" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Polo Shirts products found</p>}
              </>
            )}
          </div>
        </div>
