...
const AllProductsTops = () => {
  ...
  // Category IDs for tops products
  const topsCategoryIds = [18215, 18218, 18210, 18216, 18199, 18198, 18209, 18217, 18200];
  ...
  // Helper function to get Tops products by category ID
  const getTopsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with specific category IDs
      if (!topsCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const topsProducts = getTopsProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={AssetBannerTops} alt="Hero Tops" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Tops products found</p>}
              </>
            )}
          </div>
        </div>
