...
const SweatersTops = () => {
  ...
  // Category ID for sweaters products
  const sweatersCategoryId = [18217];
  ...
  // Helper function to get Sweaters products by category ID
  const getSweatersProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with sweaters category ID
      if (!sweatersCategoryId.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sweatersProducts = getSweatersProducts(response.data.data);
        setProducts(sweatersProducts);
      } else {
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Sweaters products found</p>}
              </>
            )}
          </div>
        </div>
