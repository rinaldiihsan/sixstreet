  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        <img src="/hero-categories.png" alt="Hero Categories" className="w-full h-full md:h-auto mb-6" />
        {/* Filter  */}
        <div className="w-full flex justify-between mb-6 sticky top-[70px] bg-white z-[997] py-1 md:py-4">
          <div className="flex flex-grow">
            ...
            <div className="border-t border-b border-r lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              <p className="font-overpass capitalize">{products.flatMap((item) => item.variants).length} Hasil</p>
            </div>

            {/* Dropdown section hidden on mobile */}
            <div className="relative border border-[#E5E5E5] hidden md:flex items-center justify-center w-full md:w-[25rem] px-4 md:px-10 py-5 gap-x-5">
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 w-full bg-white border border-[#E5E5E5] z-10">
                  <p className="font-overpass px-4 md:px-10 py-5 hover:bg-gray-200 cursor-pointer" onClick={() => handleOptionSelect('Relevance')}>
                    Relevance
                  </p>
                  <p className="font-overpass px-4 md:px-10 py-5 hover:bg-gray-200 cursor-pointer" onClick={() => handleOptionSelect('Harga Tertinggi')}>
                    Harga Tertinggi
                  </p>
                  <p className="font-overpass px-4 md:px-10 py-5 hover:bg-gray-200 cursor-pointer" onClick={() => handleOptionSelect('Harga Terendah')}>
                    Harga Terendah
                  </p>
                  <p className="font-overpass px-4 md:px-10 py-5 hover:bg-gray-200 cursor-pointer" onClick={() => handleOptionSelect('Product Terbaru')}>
                    Product Terbaru
                  </p>
                  <p className="font-overpass px-4 md:px-10 py-5 hover:bg-gray-200 cursor-pointer" onClick={() => handleOptionSelect('Alphabet')}>
                    Alphabet
                  </p>
                </div>
              )}
          ...
          {isSidebarOpen && (
            ...
            >
              ...
              <div className="mb-6">
                ...
                <ul className="mt-3 space-y-2">
                  <li className="flex items-center gap-x-2">
                    <input type="checkbox" className="border border-[#E5E5E5]" name="category" id="category1" />
                    <label className="font-overpass" htmlFor="category1">
                      Unisex
                    </label>
                  </li>
                  <li className="flex items-center gap-x-2">
                    <input type="checkbox" className="border border-[#E5E5E5]" name="category" id="category2" />
                    <label className="font-overpass" htmlFor="category2">
                      Men
                    </label>
                  </li>
                  <li className="flex items-center gap-x-2">
                    <input type="checkbox" className="border border-[#E5E5E5]" name="category" id="category3" />
                    <label className="font-overpass" htmlFor="category3">
                      Women
                    </label>
                  </li>
              ...
              <>
                  ...
                  .map((variant, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      <Link to={`/product-detail/${variant.item_id}`}>
                        <img
                          src={variant.parentThumbnail || variant.thumbnail || '/dummy-product.png'}
                ...
                {Object.values(
                  ...
                  .map((variant, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      <Link to="/">
                        <img src={variant.parentThumbnail || variant.thumbnail || '/dummy-product.png'} alt={variant.item_name} className="w-full h-[10rem] sm:h-[12rem] md:h-[15rem] lg:h-[18rem] object-cover" />
                      </Link>
                      <div className="flex flex-col text-center gap-y-2 w-full">
                        <h2 className="uppercase font-overpass font-extrabold text-sm md:text-base text-red-600">{variant.item_name}</h2>
                        <h2 className="uppercase font-overpass text-sm md:text-base text-red-600">Sold Out</h2>
                      </div>
                    </div>
                  ))}
              </>
