  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        <img src="/hero-categories.png" alt="Hero Categories" className="w-full h-auto mb-6" />
        {/* Filter  */}
        <div className="w-full flex justify-between mb-6 sticky top-[70px] bg-white z-[997] py-2 md:py-4">
                  ...
                  .map((variant, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      <Link to={`/product-detail/${variant.item_id}`}>
                        <img src={variant.parentThumbnail || variant.thumbnail || '/dummy-product.png'} alt={variant.item_name} className="w-[30rem]" />
                      </Link>
                      <div className="flex flex-col text-center gap-y-2">
