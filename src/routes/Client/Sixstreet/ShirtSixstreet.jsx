...
const ShirtSixstreet = () => {
  ...
  // Helper function to get SIXSTREET Shirt products
  const getSixstreetShirtProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only SIXSTREET Shirt products
      const productName = product.nama_produk.toLowerCase();
      if (!productName.includes('sixstreet') || !productName.includes('shirt')) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sixstreetShirtProducts = getSixstreetShirtProducts(response.data.data);
        setProducts(sixstreetShirtProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={assetHeroShirts} alt="Hero Shirt" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {filteredProducts.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No SIXSTREET Shirt products found</p>}
              </>
            )}
          </div>
        </div>
