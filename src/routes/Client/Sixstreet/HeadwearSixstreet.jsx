...
import assetHeroHeadwear from '../../../assets/banner/Headwear (2).webp';

const HeadwearSixstreet = () => {
  ...
  // Helper function to get SIXSTREET Headwear products
  const getSixstreetHeadwearProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only SIXSTREET Headwear products
      const productName = product.nama_produk.toLowerCase();
      if (!productName.includes('sixstreet') || (!productName.includes('cap') && !productName.includes('hat') && !productName.includes('beanie'))) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sixstreetHeadwearProducts = getSixstreetHeadwearProducts(response.data.data);
        setProducts(sixstreetHeadwearProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={assetHeroHeadwear} alt="Hero Headwear" className="w-full h-auto mb-6" />
              ...
              <>
                ...
                {availableProducts.map((product, index) => (
                  <div key={`available-${product.item_group_id}-${index}`} className="flex flex-col gap-y-5 items-center">
                    ...
                    <div className="flex flex-col items-center text-center w-full px-2">
                      <h2 className="uppercase font-overpass font-extrabold text-base md:text-lg break-words text-center w-full max-w-[10rem] mobileS:max-w-[10.5rem] mobile:max-w-[11.5rem] md:max-w-[23rem]">{product.base_name}</h2>
                      <h2 className="uppercase font-overpass text-sm mobile:text-base md:text-xl mt-1">{formatPrice(product.price)}</h2>
                    </div>
                  </div>
                ))}
                ...
                {soldOutProducts.map((product, index) => (
                  <div key={`soldout-${product.item_group_id}-${index}`} className="flex flex-col gap-y-5 items-center">
                    ...
                    <div className="flex flex-col items-center text-center w-full px-2">
                      <h2 className="uppercase font-overpass font-extrabold text-base md:text-lg break-words text-center w-full max-w-[10rem] mobileS:max-w-[10.5rem] mobile:max-w-[11.5rem] md:max-w-[23rem] text-red-700">
                        {product.base_name}
                      </h2>
                ...
                {/* No products message */}
                {filteredProducts.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No SIXSTREET headwear products found</p>}
              </>
            )}
          </div>
        </div>
...
export default HeadwearSixstreet;
