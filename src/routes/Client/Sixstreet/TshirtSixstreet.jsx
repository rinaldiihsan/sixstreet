  ...
  const fetchProducts = async (token) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const sixstreetProducts = data.filter((item) => item?.variants?.some((variant) => variant?.item_name?.toLowerCase().includes('sixstreet tee')));
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src="/hero-tshirt.png" alt="Hero Tshirt" className="w-full h-full md:h-auto mb-6" />
        {/* Filter  */}
        <div className="w-full flex justify-between mb-6 sticky top-[70px] bg-white z-[997] py-1 md:py-4">
          <div className="flex flex-grow">
            ...
            <div className="border-t border-b border-r lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              <p className="font-overpass capitalize">{products.filter((item) => [5472, 999, 1013, 12780, 12803].includes(item.item_category_id)).flatMap((item) => item.variants).length} Hasil</p>
            </div>
        ...
        <div className="w-full flex justify-between md:gap-x-3 overflow-x-hidden">
            ...
            <div className="mb-6">
              ...
              <ul className="mt-3 space-y-2">
                <li className="flex items-center gap-x-2">
                  <input type="checkbox" className="border border-[#E5E5E5] focus:outline-none focus:shadow-outline focus:border-[#E5E5E5] focus:ring-0" name="brand" id="AAPE" value="WUKONG" onChange={handleBrandChange} />
                  <label className="font-overpass" htmlFor="WUKONG">
                    WUKONG
                  </label>
                </li>
                <li className="flex items-center gap-x-2">
                  <input type="checkbox" className="border border-[#E5E5E5] focus:outline-none focus:shadow-outline focus:border-[#E5E5E5] focus:ring-0" name="brand" id="AAPE" value="JAMESON" onChange={handleBrandChange} />
          ...
          <div className="w-[85%] flex flex-col gap-y-8 md:flex-row md:flex-wrap md:justify-between mb-10 overflow-y-auto h-[calc(100vh-4rem)] px-5">
            ...
            ) : loginStatus === 'success' && products.some((item) => item.variants.some((variant) => variant.item_name.toLowerCase().includes('sixstreet tee'))) ? (
              <>
                ...
                {Object.values(
                  products
                    .filter((item) =>
                      item.variants.some(
                        (variant) => variant.item_name.toLowerCase().includes('sixstreet tee') && variant.available_qty > 1 // Check for stock > 1
                      )
                    )
                    .map((item) => ({
                      ...item.variants[0],
                  ...
                  .filter((item) => isProductMatchSelectedBrands(item.item_name.toLowerCase(), selectedBrands, selectedSizes))
                  .map((item, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      <Link to={`/product-detail/${item.item_group_id}`}>
                        {item?.thumbnail ? (
                          ...
                          <img src="/dummy-product.png" alt={item?.item_name || 'Product Image'} className="w-[30rem] object-cover" />
                        )}
                      </Link>
                      <div className="flex flex-col text-center gap-y-2">
                        <h2 className="uppercase font-overpass font-extrabold text-xl md:w-[24rem]">{item.item_name}</h2>
                        <h2 className="uppercase font-overpass text-xl">Rp. {item.sell_price.toLocaleString('id-ID')}</h2>
                      </div>
                    </div>
                  ))}
                ...
                {Object.values(
                  products
                    ...
                    .filter((variant) => variant.item_name.toLowerCase().includes('sixstreet tee'))
                  ...
                  .filter(
                    (variant) => variant.sell_price !== null && variant.sell_price !== 0 && variant.available_qty <= 1 // Changed from <= 0 to <= 1
                  )
                  .map((variant, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      ...
                      <div className="flex flex-col text-center gap-y-2">
                        ...
                        <h2 className="uppercase font-overpass text-xl text-red-600">{variant.available_qty === 1 ? 'Last Stock' : 'Sold Out'}</h2>
                      </div>
                    </div>
                  ))}
              </>
