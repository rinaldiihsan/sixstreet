...
const TshirtSixstreet = () => {
  ...
  // Helper function to get SIXSTREET T-shirt products
  const getSixstreetTshirtProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only SIXSTREET T-shirt products
      const productName = product.nama_produk.toLowerCase();
      if (!productName.includes('sixstreet') || (!productName.includes('tee') && !productName.includes('l/s') && !productName.includes('t-shirt'))) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sixstreetTshirtProducts = getSixstreetTshirtProducts(response.data.data);
        setProducts(sixstreetTshirtProducts);
      } else {
