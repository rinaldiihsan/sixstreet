  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={assetHeroTshirt} alt="Hero Tshirt" className="w-full h-full md:h-auto mb-6" />
        {/* Filter  */}
        <div className="w-full flex justify-between mb-6 sticky top-[70px] bg-white z-[997] py-1 md:py-4">
          <div className="flex flex-grow">
            ...
            <div className="border-t border-b border-r lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              <p className="font-overpass capitalize">{products.flatMap((item) => item.variants).filter((variant) => variant.item_name.toUpperCase().includes('SIXSTREET TEE')).length} Hasil</p>
            </div>
          ...
          <div className="w-full md:w-[85%] grid grid-cols-2 gap-5 md:grid-cols-3 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {Object.values(
                  products
                    .filter((item) => item.variants.some((variant) => variant.item_name.toLowerCase().includes('sixstreet tee') && variant.available_qty > 1))
                    .map((item) => ({
                      ...item.variants[0],
                ...
                {Object.values(
                  ...
                  .filter((variant) => variant.sell_price !== null && variant.sell_price !== 0 && variant.available_qty < 1)
                  .map((variant, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      ...
                      <div className="flex flex-col text-center gap-y-2">
                        <h2 className="uppercase font-overpass font-extrabold text-base md:text-lg break-words text-center w-full max-w-[10rem] mobileS:max-w-[10.5rem] mobile:max-w-[11.5rem] md:max-w-[23rem] text-red-600">
                          {variant.item_name}
                        </h2>
                        <h2 className="uppercase font-overpass text-sm mobile:text-base md:text-xl mt-1 text-red-600">{variant.available_qty === 1 ? 'Last Stock' : 'Sold Out'}</h2>
                      </div>
                    </div>
                  ))}
              </>
