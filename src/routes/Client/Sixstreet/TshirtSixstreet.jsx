      ...
      if (response.status === 200) {
        ...
        const sixstreetProducts = data.filter((item) =>
          item?.variants?.some((variant) => {
            ...
            return name.includes('sixstreet tee') || name.includes('sixstreet l/s');
          })
        );
                ...
                {Object.values(
                  products
                    .filter((item) =>
                      item.variants.some((variant) => {
                        const name = variant.item_name.toLowerCase();
                        return (name.includes('sixstreet tee') || name.includes('sixstreet l/s')) && variant.available_qty >= 1;
                      })
                    )
                    .map((item) => ({
                      ...item.variants[0],
                ...
                {Object.values(
                  products
                    .filter((item) =>
                      item.variants.some((variant) => {
                        const name = variant.item_name.toLowerCase();
                        return (name.includes('sixstreet tee') || name.includes('sixstreet l/s')) && variant.available_qty <= 0;
                      })
                    )
                    .slice(0, isSoldProducts) // Batasi jumlah produk sold out
                    .flatMap((item) => ({
                      ...item.variants[0],
                      item_group_id: item.item_group_id,
                      parentThumbnail: item.thumbnail,
