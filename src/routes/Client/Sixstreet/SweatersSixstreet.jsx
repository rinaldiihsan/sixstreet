...
const SweatersSixstreet = () => {
  ...
  // Helper function to get SIXSTREET Sweater products
  const getSixstreetSweaterProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only SIXSTREET Sweater products
      const productName = product.nama_produk.toLowerCase();
      if (!productName.includes('sixstreet') || (!productName.includes('sweater') && !productName.includes('hoodie') && !productName.includes('jacket'))) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sixstreetSweaterProducts = getSixstreetSweaterProducts(response.data.data);
        setProducts(sixstreetSweaterProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={assetHeroSweaters} alt="Hero Sweater" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {filteredProducts.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No SIXSTREET Sweater products found</p>}
              </>
            )}
          </div>
        </div>
