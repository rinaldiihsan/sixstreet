...
const LifestyleSixstreet = () => {
  ...
  // Helper function to get Lifestyle products by category IDs
  const getLifestyleProducts = (products, categoryIds = [7410, 15995, 17884, 12329, 12746, 15067]) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const lifestyleProducts = getLifestyleProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerLifestyle} alt="Hero Lifestyle" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="max-w-[115rem] mx-5 md:mx-auto flex gap-8 flex-row flex-wrap justify-center mb-10 md:px-5 lg:px-8 lg:gap-8">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {filteredProducts.length === 0 && !isLoading && <p className="text-center w-full">No lifestyle products found</p>}
              </>
            )}
          </div>
        </div>
...
export default LifestyleSixstreet;
