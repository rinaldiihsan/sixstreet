...
const HoodiesSixstreet = () => {
  ...
  // Helper function to get SIXSTREET Hoodie products
  const getSixstreetHoodieProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only SIXSTREET Hoodie products
      const productName = product.nama_produk.toLowerCase();
      if (!productName.includes('sixstreet') || (!productName.includes('hoodie') && !productName.includes('sweatshirt'))) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sixstreetHoodieProducts = getSixstreetHoodieProducts(response.data.data);
        setProducts(sixstreetHoodieProducts);
      } else {
  ...
  // Get image URL for products (same as Home.jsx)
    ...
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <div className="w-full flex flex-col items-center justify-center lg:justify-between md:gap-x-3 overflow-x-hidden">
          {/* Product Grid - Using same styling as Home.jsx */}
          <div className="max-w-[115rem] mx-5 md:mx-auto flex gap-8 flex-row flex-wrap justify-center mb-10 md:px-5 lg:px-8 lg:gap-8">
            {isLoading ? (
              ...
              <>
                {/* Available Products - Using same styling as Home.jsx */}
                ...
                {/* Sold Out Products - Using same styling as Home.jsx */}
                ...
                {/* No products message */}
                {filteredProducts.length === 0 && !isLoading && <p className="text-center w-full">No SIXSTREET hoodie products found</p>}
              </>
            )}
          </div>
        </div>
