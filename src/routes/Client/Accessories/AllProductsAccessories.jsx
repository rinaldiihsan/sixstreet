...
const AllProductsAccessories = () => {
  ...
  // Category IDs for accessories products
  const accessoriesCategoryIds = [7339, 7340, 17860, 1147, 7332, 1143, 10217, 36, 5516, 10230, 12622, 12628, 12632, 15320, 18225, 2835, 10222, 12621, 12626, 12631, 24806, 24807, 1149, 12610, 12611, 10219, 17866, 1139, 1137, 12432, 5429];

  // Excluded product names
  const excludedProducts = ['SIX6STREET PAPER BAG', 'SIXSTREET RING BOX WOOD BLUE', 'PLASTICBAG SIXSTREET.CO.ID'];
  ...
  // Helper function to get Accessories products by category ID
  const getAccessoriesProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with accessories category IDs
      if (!accessoriesCategoryIds.includes(product.category_id)) {
        return acc;
      }

      // Filter out excluded products
      if (excludedProducts.includes(product.nama_produk.trim().toUpperCase())) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const accessoriesProducts = getAccessoriesProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={AssetBannerAccessories} alt="Hero Accessories" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Accessories products found</p>}
              </>
            )}
          </div>
        </div>
