...
const Hats = () => {
  ...
  const hatCategoryIds = [7339, 1147];
  ...
  const getHatProducts = (products) => {
    const groupedProducts = products.reduce((acc, product) => {
      if (!hatCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const hatProducts = getHatProducts(response.data.data);
        setProducts(hatProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src="/hero-hats.png" alt="Hero Hats" className="w-full h-auto mb-6" />
