  ...
  const fetchProducts = async (token) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const sixstreetProducts = data.filter((item) => [7410, 15995, 17884, 12329, 12746, 15067].includes(item.item_category_id));
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerLifestyle} alt="Hero Hoodies" className="w-full h-full md:h-auto mb-6" />
        {/* Filter  */}
        <div className="w-full flex justify-between mb-6 sticky top-[70px] bg-white z-[997] py-1 md:py-4">
          <div className="flex flex-grow">
            ...
            <div className="border-t border-b border-r lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              <p className="font-overpass capitalize">{products.filter((item) => [7410, 15995, 17884, 12329, 12746, 15067].includes(item.item_category_id)).flatMap((item) => item.variants).length} Result</p>
            </div>
        ...
        <div className="w-full flex justify-between md:gap-x-3 overflow-x-hidden">
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-3 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            ...
            ) : loginStatus === 'success' && products.some((item) => [7410, 15995, 17884, 12329, 12746, 15067].includes(item.item_category_id)) ? (
              <>
                {/* Produk yang tersedia */}
                {Object.values(
                  products
                    .filter((item) => [7410, 15995, 17884, 12329, 12746, 15067].includes(item.item_category_id))
                    .flatMap((item) => ({
                      ...item.variants[0],
