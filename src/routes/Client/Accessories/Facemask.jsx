  ...
  const fetchProducts = async (token) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const sixstreetProducts = data.filter((item) => [18100].includes(item.item_category_id));
    ...
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <div className="w-full flex justify-between mb-6 sticky top-[72px] bg-white z-[997] py-4">
          <div className="flex flex-grow">
            ...
            <div className="border lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              {/* <p className="font-overpass capitalize">{products.filter((item) => item.item_category_id === 18100).flatMap((item) => item.variants).length} Result</p> */}
            </div>
        ...
        <div className="w-full flex justify-between gap-x-3">
          {/* Product */}
          <div lassName="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
            ...
            ) : loginStatus === 'success' && products.some((item) => item.item_category_id === 18100) ? (
              <>
                {/* Produk yang tersedia */}
                {Object.values(
                  products
                    .filter((item) => item.item_category_id === 18100)
