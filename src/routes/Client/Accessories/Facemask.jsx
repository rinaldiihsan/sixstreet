...
const Facemask = () => {
  ...
  const facemaskCategoryIds = [18100];
  ...
  const getFacemaskProducts = (products) => {
    const groupedProducts = products.reduce((acc, product) => {
      if (!facemaskCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const facemaskProducts = getFacemaskProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src="/hero-facemask.png" alt="Hero Facemask" className="w-full h-auto mb-6" />
            ...
            {isLoading ? (
              Array.from({ length: 8 }).map((_, index) => (
                <div key={index} className="flex flex-col gap-y-5 items-center">
                  <Skeleton className="w-[8rem] h-[8rem] md:w-[18rem] md:h-[18rem] lg:w-[13rem] lg:h-[13rem] laptopL:w-[22rem] laptopL:h-[22rem] object-cover" />
                  ...
                  <div key={`available-${product.item_group_id}-${index}`} className="flex flex-col gap-y-5 items-center">
                    <Link to={`/product-detail/${product.item_group_id}`}>
                      <img
                        ...
                        className="w-[8rem] h-[8rem] md:w-[18rem] md:h-[18rem] lg:w-[13rem] lg:h-[13rem] laptopL:w-[22rem] laptopL:h-[22rem] object-cover"
                    ...
                    <div className="flex flex-col items-center text-center w-full px-2">
                      <h2 className="uppercase font-overpass font-medium lg:font-semibold text-sm md:text-lg lg:text-base text-center">{product.base_name}</h2>
                ...
                {soldOutProducts.map((product, index) => (
                  <div key={`soldout-${product.item_group_id}-${index}`} className="flex flex-col gap-y-5 items-center">
                    <Link to={`/product-detail-sold/${product.item_group_id}`} className="cursor-pointer transition-opacity duration-300 hover:opacity-75">
                      <img
                        src={getProductImageUrl(product)}
                        className="w-[8rem] h-[8rem] md:w-[18rem] md:h-[18rem] lg:w-[13rem] lg:h-[13rem] laptopL:w-[22rem] laptopL:h-[22rem] object-cover opacity-50"
                        alt={product.base_name}
                        onError={(e) => {
                          e.target.src = '/dummy-product.png';
                        }}
                    ...
                    <div className="flex flex-col items-center text-center w-full px-2">
                      <h2 className="uppercase font-overpass font-medium lg:font-semibold text-sm md:text-lg lg:text-base text-center text-red-700">{product.base_name}</h2>
