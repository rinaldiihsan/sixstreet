  ...
  const fetchProducts = async (token) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const sixstreetProducts = data.filter((item) => [12610, 12611, 10219].includes(item.item_category_id));
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={assetBannerWallet} alt="Hero Wallets" className="w-full h-auto mb-6" />
        {/* Filter  */}
          ...
          <div className="flex flex-grow">
            {/* <div className="border border-[#E5E5E5] hidden items-center justify-center w-[17rem] px-10 py-5 gap-x-14">
              ...
              <svg width="24" height="24" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M4.12611, 10219.41666H1.6665" stroke="#292D32" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
                <path
            ...
            <div className="border lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              {/* <p className="font-overpass capitalize">{products.filter((item) => [12610, 12611, 10219].includes(item.item_category_id)).flatMap((item) => item.variants).length} Result</p> */}
            </div>
        ...
        <div className="w-full flex justify-between gap-x-3">
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            ...
            ) : loginStatus === 'success' && products.some((item) => [12610, 12611, 10219].includes(item.item_category_id)) ? (
              <>
                {/* Produk yang tersedia */}
                {Object.values(
                  products
                    .filter((item) => [12610, 12611, 10219].includes(item.item_category_id))
                    .flatMap((item) => ({
                      ...item.variants[0],
