...
const Belts = () => {
  ...
  // Category ID for belts products
  const beltsCategoryIds = [12468];
  ...
  // Helper function to get Belts products by category ID
  const getBeltsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with belts category IDs
      if (!beltsCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const beltsProducts = getBeltsProducts(response.data.data);
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Belts products found</p>}
              </>
            )}
          </div>
        </div>
