...
import assetBannerEyewear from '../../../assets/banner/Eyewear (1).webp';
...
const Eyewear = () => {
  ...
  // Category ID untuk Collectibles
  const eyewearCategoryIds = [7332, 1143];
  ...
  // Helper function to get Bag products by category ID
  const getBagProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with bag category IDs
      if (!eyewearCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const bagProducts = getBagProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center">
        ...
        <img src={assetBannerEyewear} alt="Hero Bag" className="w-full h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Bag products found</p>}
              </>
            )}
          </div>
        </div>
