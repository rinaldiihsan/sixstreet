...
const Socks = () => {
  ...
  // Category IDs for socks products
  const socksCategoryIds = [24806, 24807];
  ...
  // Helper function to get Socks products by category ID
  const getSocksProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with socks category IDs
      if (!socksCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const socksProducts = getSocksProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerSocks} alt="Hero Socks" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Socks products found</p>}
              </>
            )}
          </div>
        </div>
