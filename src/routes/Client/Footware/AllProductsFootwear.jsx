...
const AllProductsFootwear = () => {
  ...
  // Category IDs for footwear products
  const footwearCategoryIds = [5472, 999, 1013, 12780, 12803, 12595, 12808, 1009, 1033, 7560, 12794, 7545, 17895, 24806, 24807];
  ...
  // Helper function to get Footwear products by category ID
  const getFootwearProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with footwear category IDs
      if (!footwearCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const footwearProducts = getFootwearProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={AssetBannerFootwear} alt="Hero Footwear" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Footwear products found</p>}
              </>
            )}
          </div>
        </div>
