...
import assetBannerSandals from '../../../assets/banner/Sandals-1.webp';
...
const Sandals = () => {
  ...
  // Category IDs for sandals products
  const sandalsCategoryIds = [7545, 17895, 1013];
  ...
  // Helper function to get Sandals products by category ID
  const getSandalsProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with sandals category IDs
      if (!sandalsCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sandalsProducts = getSandalsProducts(response.data.data);
        setProducts(sandalsProducts);
      } else {
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerSandals} alt="Hero Sandals" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Sandals products found</p>}
              </>
            )}
          </div>
        </div>
