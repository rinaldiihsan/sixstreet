...
import assetBannerSneakers from '../../../assets/banner/Sneakers.webp';
...
const Sneakers = () => {
  ...
  // Category IDs for sneakers products
  const sneakersCategoryIds = [5472, 999, 1013, 12780, 12803];
  ...
  // Helper function to get Sneakers products by category ID
  const getSneakersProducts = (products) => {
    // Group products by base name first
    const groupedProducts = products.reduce((acc, product) => {
      // Filter only products with sneakers category IDs
      if (!sneakersCategoryIds.includes(product.category_id)) {
        return acc;
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data.success) {
        const sneakersProducts = getSneakersProducts(response.data.data);
  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        ...
        <img src={assetBannerSneakers} alt="Hero Sneakers" className="w-full h-full md:h-auto mb-6" />
          ...
          <div className="w-full grid grid-cols-2 gap-5 lg:grid-cols-4 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden scroll-hidden">
            {isLoading ? (
              ...
              <>
                ...
                {/* No products message */}
                {products.length === 0 && !isLoading && <p className="uppercase font-overpass font-bold text-xl text-center w-full col-span-full">No Sneakers products found</p>}
              </>
            )}
          </div>
        </div>
