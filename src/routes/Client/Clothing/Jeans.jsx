  ...
  return (
    <>
      <div className="mt-20 max-w-[115rem] py-5 mx-auto px-5 md:px-2 flex flex-col justify-center items-center overflow-x-hidden">
        <img src="/hero-jeans.png" alt="Hero Jeans" className="w-full h-full md:h-auto mb-6" />
        {/* Filter  */}
        <div className="w-full flex justify-between mb-6 sticky top-[70px] bg-white z-[997] py-1 md:py-4">
          <div className="flex flex-grow">
            ...
            <div className="border-t border-b border-r lg:border-r-0 border-[#E5E5E5] flex-grow flex items-center px-4 md:px-10 py-5">
              <p className="font-overpass capitalize">{products.filter((item) => item.item_category_id === 911).flatMap((item) => item.variants).length} Hasil</p>
            </div>
        ...
        <div className="w-full flex justify-between md:gap-x-3 overflow-x-hidden">
            ...
            <div className="mb-6">
              ...
              <ul className="mt-3 space-y-2">
                <li className="flex items-center gap-x-2">
                  <input type="checkbox" className="border border-[#E5E5E5] focus:outline-none focus:shadow-outline focus:border-[#E5E5E5] focus:ring-0" name="brand" id="AAPE" value="AAPE" onChange={handleBrandChange} />
                  <label className="font-overpass" htmlFor="AAPE">
                    AAPE
                  </label>
                </li>
                <li className="flex items-center gap-x-2">
          ...
          <div className="w-full md:w-[85%] grid grid-cols-2 gap-5 md:grid-cols-3 mb-10 overflow-y-auto h-[calc(100vh-4rem)] md:px-5 overflow-x-hidden">
            {isLoading ? (
            ...
            ) : loginStatus === 'success' && products.some((item) => item.item_category_id === 911) ? (
              <>
                {/* Produk yang tersedia */}
                {Object.values(
                  products
                    .filter((item) => item.item_category_id === 911)
                  ...
                  .map((variant, index) => (
                    <div key={index} className="flex flex-col gap-y-5 items-center">
                      ...
                      <div className="flex flex-col text-center gap-y-2 w-full">
                        <h2 lassName="uppercase font-overpass font-extrabold  md:text-xl w-[10rem] mobileS:w-[10.5rem] mobile:w-[11.5rem] md:w-[24rem]">{variant.item_name}</h2>
              ...
              <p className="uppercase font-overpass font-bold">{error ? `Login failed: ${error}` : 'No products found in this category'}</p>
            )}
          </div>
        </div>
      </div>
